...
from fastapi import (
    ...
    status,
    Query,
)  # Removed Body as it wasn't directly used
...
async def create_any_user(
    user_in: AdminUserCreate, current_admin: User = Depends(get_current_admin_user)
):
    """
    ...
    """
    logger.info(
        f"Admin '{current_admin.email}' attempting to create user: '{user_in.email}' with type: '{user_in.user_type}'"
    )
    ...
    if existing_user:
        logger.warning(
            f"Failed to create user: Email '{user_in.email}' already exists."
        )
        raise HTTPException(
    ...
    try:
        ...
        logger.exception(
            "Password hashing failed during user creation for email %s.", user_in.email
        )
    ...
    new_user = User(
        **new_user_data,  # Pass validated data
        ...
        is_active=(
            user_in.is_active if user_in.is_active is not None else True
        ),  # Use schema default or True
    ...
    try:
        ...
        if not inserted_user or not inserted_user.id:
            logger.critical(
                "CRITICAL: Database insert reported success but user object/ID is missing for email %s",
                user_in.email,
            )
            raise HTTPException(
                status_code=500, detail="Failed to confirm user creation after insert."
            )

        logger.info(
            f"Admin '{current_admin.email}' created user '{inserted_user.email}' (ID: {inserted_user.id})"
        )
        ...
        try:
            ...
            logger.error(
                "Failed to create default ParentalSettings for user %s: %s",
                inserted_user.id,
                e_parental,
                exc_info=True,
            )
            ...
            logger.warning(
                f"Rolled back creation of user {inserted_user.id} due to ParentalSettings failure."
            )
            raise HTTPException(
                status_code=500,
                detail="Failed to initialize user settings (Parental). User creation rolled back.",
            )
        ...
        try:
            ...
            logger.info(
                f"Created default AppearanceSettings for user {inserted_user.id}"
            )
        except Exception as e_appearance:
            logger.error(
                "Failed to create default AppearanceSettings for user %s: %s",
                inserted_user.id,
                e_appearance,
                exc_info=True,
            )
            ...
            await inserted_user.delete()
            logger.warning(
                f"Rolled back creation of user {inserted_user.id} and ParentalSettings due to AppearanceSettings failure."
            )
            raise HTTPException(
                status_code=500,
                detail="Failed to initialize user settings (Appearance). User creation rolled back.",
            )

    except Exception as e_user:
        # Catch potential database errors during user insert itself
        logger.exception(
            "Database error during user insertion for email %s.", user_in.email
        )
        ...
        if inserted_user and hasattr(inserted_user, "delete"):
            logger.warning(
                "Attempting cleanup for potentially partially inserted user %s",
                inserted_user.id or user_in.email,
            )
            await inserted_user.delete()  # Requires inserted_user to be a valid Beanie doc
        raise HTTPException(
            status_code=500, detail="Database error during user registration."
        )
    ...
    user_read_data = {
        ...
        "avatar_uri": (
            str(inserted_user.avatar_uri) if inserted_user.avatar_uri else None
        ),  # Ensure HttpUrl is stringified if needed
...
async def list_users(
    ...
    limit: int = Query(
        100, ge=1, le=500, description="Maximum number of records per page"
    ),
    ...
    email_search: Optional[str] = Query(
        None, description="Search by partial email match (case-insensitive)"
    ),
    name_search: Optional[str] = Query(
        None, description="Search by partial name match (case-insensitive)"
    ),
    current_admin: User = Depends(get_current_admin_user),
):
    """Lists users with pagination and filtering options. Requires ADMIN privileges."""
    logger.info(
        f"Admin '{current_admin.email}' requested user list: skip={skip}, limit={limit}, type={user_type}, status={status}, email='{email_search}', name='{name_search}'"
    )

    query_filter = {}
    if user_type:
        query_filter[User.user_type] = user_type  # Use model field for clarity
    if status:
        query_filter[User.status] = status
    if email_search:
        query_filter[User.email] = {"$regex": email_search, "$options": "i"}
    if name_search:
        query_filter[User.name] = {"$regex": name_search, "$options": "i"}

    try:
        users_cursor = (
            User.find(query_filter).sort(-User.created_at).skip(skip).limit(limit)
        )
        ...
        for user in users_list:
            if user.id:
                ...
                logger.error(
                    f"User found in list query (filter: {query_filter}) but missing ID: email='{getattr(user, 'email', 'N/A')}'"
                )

        logger.debug(
            f"Returning {len(response_data)} users for admin list request by '{current_admin.email}'."
        )
...
async def get_user_by_id(
    user_id: str, current_admin: User = Depends(get_current_admin_user)
):
    """Retrieves a specific user by ID. Requires ADMIN privileges."""
    logger.info(
        f"Admin '{current_admin.email}' requesting details for user ID: {user_id}"
    )
    try:
        ...
        if not user:
            logger.warning(
                f"User ID '{user_id}' not found (Admin request: '{current_admin.email}')"
            )
            raise HTTPException(
        ...
        if not user.id:
            logger.error(
                f"User fetched for ID {user_id} but has no .id attribute (data inconsistency?)"
            )
            raise HTTPException(
                status_code=500,
                detail="Error retrieving user details - inconsistent data.",
            )
        ...
        logger.debug(
            f"Successfully retrieved user details for ID: {user_id} for admin '{current_admin.email}'"
        )
        ...
        logger.exception(
            f"Error fetching user by ID {user_id} for admin '{current_admin.email}':"
        )
...
async def update_user_by_admin(
    ...
    logger.info(
        f"Admin '{current_admin.email}' attempting update for user ID: {user_id}"
    )

    user_to_update = await User.get(user_id)
    if not user_to_update:
        logger.warning(
            f"Update failed: User ID '{user_id}' not found (Admin: '{current_admin.email}')."
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found"
        )
    ...
    if not update_data:
        logger.warning(
            f"Update request for user {user_id} by admin '{current_admin.email}' had no data to update."
        )
        raise HTTPException(
    ...
    try:
        ...
        for field, value in update_data.items():
            if hasattr(user_to_update, field):
                ...
                if getattr(user_to_update, field) != value:
                    ...
                    logger.debug(
                        f"Skipping update for user {user_id}: Field '{field}' value unchanged ('{value}')."
                    )
            else:
                # Log attempts to update fields not present on the model (might indicate schema mismatch)
                logger.warning(
                    f"Attempted to update non-existent field '{field}' on User model for ID {user_id}"
                )
        ...
        if updated_fields_count > 0:
            ...
            logger.info(
                f"Admin '{current_admin.email}' successfully updated {updated_fields_count} field(s) for user ID: {user_id}"
            )
            updated_user = (
                user_to_update  # Use the instance potentially modified by save()
            )
        else:
            logger.info(
                f"No effective changes applied for user {user_id} by admin '{current_admin.email}'. Save skipped."
            )
            updated_user = user_to_update  # Return the existing user data unchanged

        if not updated_user or not updated_user.id:
            logger.error(
                f"User object invalid after update attempt for ID '{user_id}'."
            )
            raise HTTPException(
                status_code=500, detail="Failed to process user data after update."
            )
        ...
        user_read_data = {
            ...
            "avatar_uri": (
                str(updated_user.avatar_uri) if updated_user.avatar_uri else None
            ),
        ...
        ValidationError
    ) as e:  # Catch Pydantic errors during setattr or save validation
        logger.warning(
            f"Validation error during user update for ID {user_id} by admin '{current_admin.email}': {e.errors()}",
            exc_info=False,
        )  # Don't need full trace usually
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=e.errors()
        )
    except RevisionIdWasChanged:  # Catch Beanie specific error for concurrent updates
        logger.warning(
            f"Concurrency error: User {user_id} was modified during update attempt by admin '{current_admin.email}'."
        )
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
            detail="User data was modified concurrently. Please refresh and retry.",
        )
    except Exception as e:
        logger.exception(
            f"Error updating user ID {user_id} by admin '{current_admin.email}':"
        )
...
async def delete_user(
    ...
    logger.warning(
        f"Admin '{current_admin.email}' attempting to DELETE user ID: {user_id}"
    )

    # Prevent self-deletion
    if str(current_admin.id) == user_id:
        logger.error(
            f"Admin '{current_admin.email}' (ID: {current_admin.id}) attempted self-deletion."
        )
        raise HTTPException(
    ...
    if not user_to_delete:
        logger.warning(
            f"Deletion failed: User ID '{user_id}' not found (Admin: '{current_admin.email}')."
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="User not found"
        )
    ...
    if not user_to_delete.id:
        logger.error(
            f"Found user for deletion (ID: {user_id}) but instance is missing '.id'. Aborting deletion."
        )
        raise HTTPException(
            status_code=500, detail="Internal error retrieving user ID for deletion."
        )
    ...
    try:
        ...
        deleted_parental_count = (
            deleted_parental_result.deleted_count if deleted_parental_result else 0
        )
        logger.info(
            f"Deleted {deleted_parental_count} ParentalSettings document(s) for user {user_id}"
        )

        deleted_appearance_result = await AppearanceSettings.find(AppearanceSettings.user.id == user_obj_id).delete()  # type: ignore
        deleted_appearance_count = (
            deleted_appearance_result.deleted_count if deleted_appearance_result else 0
        )
        logger.info(
            f"Deleted {deleted_appearance_count} AppearanceSettings document(s) for user {user_id}"
        )
        ...
        if delete_result and delete_result.deleted_count > 0:
            logger.info(
                f"Admin '{current_admin.email}' successfully DELETED user ID: {user_id} and associated settings."
            )
            ...
            logger.error(
                f"User document deletion failed or reported 0 deleted for ID: {user_id}. Result: {delete_result}"
            )
            # Find the user again to check if they still exist
            check_user = await User.get(user_id)
            if check_user:
                raise HTTPException(
                    status_code=500, detail="User deletion failed unexpectedly."
                )
            else:
                # User seems to be gone now, maybe concurrent deletion? Log and treat as success (idempotency).
                logger.warning(
                    f"User {user_id} not found after delete operation reported 0 count. Assuming already deleted."
                )
                return None  # Return 204 anyway

    except Exception as e:
        logger.exception(
            f"Error during deletion process for user ID {user_id} initiated by admin '{current_admin.email}':"
        )
        # Consider more specific error handling if needed
        raise HTTPException(
            status_code=500, detail="Could not complete the user deletion process."
        )
