...
from app.db.models.enums import UserStatus, UserType, Gender # Import enums used for conversion
...
def _prepare_user_read_data(user_model: User) -> Dict[str, Any]:
    ...
    response_data = user_model.model_dump(
        exclude={'hashed_password', 'status'} # Exclude fields not in UserRead or sensitive
...
async def read_users_me(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        logger.exception(f"Error validating/preparing user data for {current_user.email}")
...
async def update_user_me(
    ...
    if not update_data:
        logger.info(f"Update request for user '{current_user.email}' had no data to update.")
        prepared_data = _prepare_user_read_data(current_user)
        return UserRead.model_validate(prepared_data)
    ...
    if updated_fields_count > 0:
        try:
            ...
            logger.info(f"Profile updated ({updated_fields_count} fields) for {current_user.email}.")
        except Exception:
        ...
        logger.info(f"No actual changes for user '{current_user.email}'. No save performed.")

    try:
        prepared_data = _prepare_user_read_data(current_user)
        return UserRead.model_validate(prepared_data)
    except Exception:
        logger.exception(f"Error validating/preparing updated user data for {current_user.email}")
        raise HTTPException(status_code=500, detail="Internal server error processing updated user data.")
...
async def deactivate_user_me(
    ...
    if not current_user.is_active and current_user.status == UserStatus.INACTIVE:
        logger.info(f"User '{current_user.email}' is already inactive. No action taken.")
        return None
