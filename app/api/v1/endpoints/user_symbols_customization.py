...
async def get_user_customized_symbols(current_user: User = Depends(get_current_active_user)):
...
async def add_symbol_to_user_category(
    ...
    category_name: str = Path(..., title="Category name (lowercase)", min_length=1, example="food")
):
    processed_category_name = category_name.lower().strip()
    new_keyword = payload.keyword.strip()

    logger.info(f"User '{current_user.email}' attempting to add symbol '{new_keyword}' to their category '{processed_category_name}'.")

    if not new_keyword:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="Symbol keyword cannot be empty.")
    ...
    existing_symbol = await UserCategorySymbol.find_one(
        ...
        UserCategorySymbol.keyword == new_keyword # Consider case-insensitivity if needed
    )
    if existing_symbol:
        logger.warning(f"User '{current_user.email}' - Symbol '{new_keyword}' already exists in their category '{processed_category_name}'.")
        raise HTTPException(
    ...
    try:
        ...
        logger.info(f"User '{current_user.email}' successfully added symbol '{new_keyword}' to category '{processed_category_name}'. ID: {new_user_symbol.id}")
        ...
        logger.exception(f"DB error adding symbol for user {current_user.email} to category {processed_category_name}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not add symbol.")
