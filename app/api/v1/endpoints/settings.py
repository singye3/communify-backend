...
def _prepare_settings_read_data(settings_model: Any, model_name: str) -> Dict[str, Any]:
    ...
    if model_name == "ParentalSettings":
        if 'asd_level' in response_data and isinstance(response_data.get('asd_level'), AsdLevelEnum):
        ...
        if 'downtime_days' in response_data and isinstance(response_data.get('downtime_days'), list):
            new_downtime_days = []
            for day_enum_val in response_data['downtime_days']:
                if isinstance(day_enum_val, DayOfWeekEnum): new_downtime_days.append(day_enum_val.value)
                elif isinstance(day_enum_val, str):
                    try: DayOfWeekEnum(day_enum_val); new_downtime_days.append(day_enum_val)
                    except ValueError: logger.warning(f"Invalid DayOfWeek string '{day_enum_val}' in DB.")
...
# ============================
@router.get("/parental", response_model=ParentalSettingsRead, summary="Get Parental Settings", tags=["Settings - Parental"])
async def read_parental_settings(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        if not settings:
            logger.info(f"No parental settings for {current_user.email}. Creating and returning defaults.")
        ...
        logger.error(f"Validation error creating ParentalSettingsRead response for {current_user.email}: {e.errors()}")
        raise HTTPException(status_code=500, detail="Internal error processing parental settings response.")
    except Exception:
...
async def patch_parental_settings(
    ...
    if not settings_values_to_update:
        logger.warning(f"No settings data in 'value' field by {current_user.email}. Returning current settings.")
    ...
    try:
        ...
        logger.error(f"Pydantic validation error constructing response for {current_user.email}: {e.errors()}")
        raise HTTPException(status_code=500, detail="Internal server error processing settings response.")
    except Exception:
        logger.exception(f"General error updating parental settings for {current_user.email}")
...
# (Keep this section as it was in the previous correct version)
# ============================
@router.get("/appearance", response_model=AppearanceSettingsRead, summary="Get Appearance Settings", tags=["Settings - Appearance"])
async def read_appearance_settings(current_user: User = Depends(get_current_active_user)):
    ...
    try:
        ...
        if not settings:
            logger.info(f"No appearance settings for {current_user.email}. Creating and returning defaults.")
            default_data_for_db = AppearanceSettingsBase().model_dump()
            settings_dict_for_create = {"user": current_user, **default_data_for_db}
            settings = AppearanceSettings(**settings_dict_for_create) # type: ignore
            await settings.insert()
            logger.info(f"Default appearance settings created for {current_user.email}.")
        ...
        logger.error(f"Validation error creating AppearanceSettingsRead response for {current_user.email}: {e.errors()}")
        raise HTTPException(status_code=500, detail="Internal error processing appearance settings response.")
    except Exception:
...
@router.patch("/appearance", response_model=AppearanceSettingsRead, summary="Update Appearance Settings", tags=["Settings - Appearance"])
async def patch_appearance_settings(settings_in: AppearanceSettingsUpdate, current_user: User = Depends(get_current_active_user)):
    # ... (implementation from previous correct version) ...
    ...
    if not update_data:
        logger.warning(f"No appearance settings data provided by {current_user.email}. Returning current.")
